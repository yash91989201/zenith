...
export async function GET(req: Request) {
  try {
    ...
    const media = await getFileFromBucket({
      bucketName: "media",
      fileName: fileId
    })

    if (!media) throw new Error("file not found")
    const imageBuffer = await streamToBuffer(media)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const agencyLogoArrayBuffer = await media.arrayBuffer()

    const fileName = `${createId()}.${getFileExtension(media)}`
    const fileUrl = `${env.NEXT_PUBLIC_URL}/api/file/media?file=${fileName}`
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const [deleteMediaDataRes] = await db.delete(MediaTable).where(eq(MediaTable.link, `${req.url}?file=${file}`))

    if (deleteMediaDataRes.affectedRows === 0) throw new Error("Media data not removed from db")

    return Response.json({ status: "SUCCESS", message: "Media file deleted" }, { status: 200 })
  } catch (error) {
    if (error instanceof Error) {
      return Response.json({ status: "FAILED", message: error.message }, { status: 400 })
    } else {
      return Response.json({ status: "FAILED", message: "Unable to delete media file" }, { status: 400 })
    }
  }
}